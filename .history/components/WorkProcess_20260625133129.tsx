...
export default function WorkProcess() {
  ...
  useEffect(() => {
    ...
    const lastTouchPoint = lastRect.top - timelineRect.top;
    setMobileLine({
  top: firstCenter,
  height: Math.max(0, lastTouchPoint - firstCenter),
});
    };
      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <div className="relative mx-auto mt-16 hidden h-[820px] max-w-[1350px] lg:block">
          ...
          {steps.map((step, idx) => (
            <div
              key={step.no}
              className="absolute z-10 w-[280px] -translate-x-1/2 text-center"
            ...
            >
              ...
              <div
                className={`mt-5 flex min-h-[190px] flex-col justify-start rounded-2xl border border-[#0d2d47]/10 bg-[#B6C4E7] p-4 font-[family:var(--font-inter)] shadow-[0_18px_40px_rgba(13,45,71,0.14)] transition-all duration-700 ${
                  glowStates[idx]
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <Fragment key={step.no}>
                  ...
                  {showDownArrow ? (
                    <div className="relative h-7 sm:h-15">
                      <div className="absolute left-5 -top-8 z-20 flex h-6 w-6 -translate-x-1/2 -translate-y-1/2 items-center justify-center  sm:left-1/2">
                        <svg
                          viewBox="0 0 24 24"
                          aria-hidden="true"
                          className="h-4 w-4"
